...
- **Decision**: Healthcare domain schema/data generated by a Python-native
  Synthea-style generator in `domains/healthcare/seed.py` — Synthea's
  canonical table/column shape (`patients`, `encounters`, `conditions`,
  etc.) populated via `Faker` plus curated ICD-10-style diagnosis-code and
  clinical-note-template pools, rather than an actual Synthea CSV/FHIR
  export (amended 2026-07-29, T012 — see tech-stack.md's "Healthcare
...
- **Alternatives considered**: Loading a pre-generated Synthea CSV export
  ...
  classifier/enforcement paths, not achieving Synthea's full clinical
  realism); running the actual Synthea Java tool at seed time (rejected —
  no JVM in this environment; would add a toolchain dependency `tech-stack.md`
  doesn't otherwise require); hand-crafting both domains' data with Faker
  alone and no Synthea-derived structure (originally rejected — Synthea's
  realistic healthcare structure is what makes Scenario 1/3 meaningful test
  cases; the Python-native generator preserves this by keeping the same
  table shape, ICD-10-style codes, and free-text clinical-narrative
  realism Synthea would produce).
