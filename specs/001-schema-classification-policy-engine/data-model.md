...
| `decision` | enum: `allow`, `block`, `mask`, `classify_auto_approved`, `classify_pending_review`, `classify_approved`, `classify_rejected`, `policy_published`, `question_not_mapped` | Both enforcement and classification decisions are logged (Principle VIII). `mask` is reserved for a future milestone — Milestone 1's policy actions are `allow`/`block`/`role_gate` only (FR-008), so no enforcement path produces `mask` yet. `policy_published` covers the `POST /policy/publish` action (contracts/api.md) — added 2026-07-30, since neither FR-010's "classification decision"/"enforcement decision" wording nor the original enum had a slot for it. `question_not_mapped` covers Scenario 10's outcome — added 2026-07-30 (T056) for the same reason: it is neither `allow` nor `block`. |
